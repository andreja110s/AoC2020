...
func CalculateYourSeatId(_ arrayOfPartitions: [String]) -> Int
{
  var sortedSeatIds = arrayOfPartitions.map { CalculateSeatId($0) }
  sortedSeatIds.sort()

  for index in 1...sortedSeatIds.count-2
  {
    if (sortedSeatIds[index] == sortedSeatIds[index + 1] + 2 
    || sortedSeatIds[index] == sortedSeatIds[index - 1] + 2)
    {
      return sortedSeatIds[index] - 1
    }
    else if (sortedSeatIds[index] == sortedSeatIds[index + 1] - 2
    || sortedSeatIds[index] == sortedSeatIds[index - 1] - 2)
    {
      return sortedSeatIds[index] + 1
    }
...
print(CalculateYourSeatId(arrayOfPartitions))